...
function groundValid() return true end -- allow in air
...
function findTargetPosition(dir, maxDist)
  ...
  while dist <= maxDist do
    ...
    if not world.polyCollision(collisionPoly, testPos, {"Null", "Block", "Dynamic"}) then
      ...
      -- if not world.polyCollision(collisionPoly, oneDown, {"Null", "Block", "Dynamic", "Platform"}) then testPos = oneDown end
    else
